    ...
    react(name, payload) {
        switch (name) {
            case REACTIONS.optional:
                //wtf
                for (let name in this.micro_snpts.selected[payload.name].optional) {
                    this.micro_snpts.selected[payload.name].optional[name] = false;
                }
                this.micro_snpts.update(payload.name, {
                    optional: this.selected.cards[payload.side]
                });
                break;
